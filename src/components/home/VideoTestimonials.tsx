  ...
  return (
    ...
    >
      {videoTestimonials.map(({ id, title }) => {
        ...
        return (
          <div key={id} className="relative aspect-[9/16] overflow-hidden rounded-[8px] bg-[#160020] shadow-[0_12px_30px_rgba(45,0,70,0.09)]">
            <div className={`absolute inset-0 grid place-items-center bg-[radial-gradient(circle_at_50%_38%,#710bc0_0%,#31004d_48%,#160020_100%)] transition-opacity duration-500 ${loaded ? "opacity-0" : "opacity-100"}`} aria-hidden="true">
              <span className="grid h-16 w-16 place-items-center rounded-full border border-white/50 bg-white/15 text-white backdrop-blur-sm">
